...
export const useActivityTracker = () => {
  ...
  useEffect(() => {
    ...
    const loadTodayActivity = () => {
      ...
      console.log(
        `Loading activity for today: ${todayStr} (Local time: ${today.toLocaleString()})`,
      );
      ...
      if (storedActivities) {
        ...
        if (todayActivity) {
          console.log(
            `Found existing activity for today: ${todayActivity.minutesActive} minutes`,
          );
    ...
    const saveActivity = () => {
      ...
      const elapsedSeconds = Math.floor(
        (new Date().getTime() - activityStartTime.getTime()) / 1000,
      );
      ...
      console.log(
        `Saving activity for ${todayStr} (Local time: ${today.toLocaleString()}): ${minutesActive} minutes`,
      );
